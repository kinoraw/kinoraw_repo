        ...
        if seq:
            ...
            if strip != None:
                if "IN" and "OUT" in markers:
                    ...
                    functions.triminout(strip,sin,sout)
                else:
...
class OBJECT_OT_Jumpprev(bpy.types.Operator):  #Operator jump previous edit point
    ...
    def invoke(self, context, event):
        scene=bpy.context.scene
        seq=scene.sequence_editor
        editpoints = functions.geteditpoints(seq)
...
class OBJECT_OT_Markerprev(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        bpy.context.scene.frame_current = functions.searchprev(scene.frame_current, markerlist)
        return {'FINISHED'}
